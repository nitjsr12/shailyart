...
const PaintingCard = ({ painting }: { painting: Painting }) => {
  const whatsappMessage = encodeURIComponent(
    `Hi, I'm interested in ordering the painting "${painting.title}".`
  );
    ...
    >
      ...
      >
        <img
          src={painting.image}
          alt={painting.title}
          className="h-full w-full object-cover transition-transform duration-image ease-out group-hover:scale-[1.05]"
        />
        {/* Depth + readability */}
        <div
      ...
      <div className="container relative z-10 mx-auto px-4 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 sm:gap-4 lg:grid-cols-3 lg:gap-5">
          {filteredPaintings.map((painting, index) => (
            <RevealOnScroll key={painting.id} delayMs={index * 55}>
